...
export const Logo: React.FC<LogoProps> = ({ className = '', showText = true, size = 'md' }) => {
  ...
  return (
    <Link 
      to="/" 
      className={`inline-flex items-center gap-3 group transition-transform duration-300 hover:scale-105 ${className}`}
    ...
    >
      ...
      {showText && (
        <div className="flex flex-col">
          <span className={`text-white font-black uppercase font-sans ${sizeMap[size].text}`}>
            REDCANVASS
          </span>
          <span className="text-[9px] uppercase tracking-[0.25em] text-[#9A9DA7] font-semibold -mt-1 group-hover:text-[#FF3154] transition-colors">
            CREATIVE STUDIO
          </span>
        </div>
      )}
